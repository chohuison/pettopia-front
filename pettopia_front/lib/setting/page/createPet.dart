...
class _CreatePetState extends State<CreatePet> {
  ...
  void onHandlePetInformation(String petNum, String petName,String wight, int breedPk, int fur, int sex, int _neutering, String birth){
    _petNumber = petNum;
    _petName = petName;
    _breedPk= breedPk;
    _fur=fur;
...
//아직 사진은 안됬음 
  void _saveButtonHandle(){
    if(_petNumber =="" || _petName == "" || _birth =="" || _parentName == "" || _parentPhoneNum ==""){
      setState(() {
                ...
                child: Column(
                  children: <Widget>[
                    ...
                    Container(
                        ...
                        child: CreatePetParentValue(petParentValueHandle: petParentValueHandle),
                        width: 350.w,
                        height: 105.h),
                        //필수 입력값 입력하지 않은 경우
                        Center(
                          child:Text(errMesg, style: TextStyle(color: Colors.red),)
                        ),
                    //등록 버튼
                    Container(
                      margin: EdgeInsets.only(top: 5.h),

...
class ViewPetAddInfo extends StatefulWidget {
  ...
  const ViewPetAddInfo(
      {Key? key, required this.petAddInfo, required this.medicen})
      : super(key: key);
          ...
          children: <Widget>[
            ...
            Container(
              ...
              child: Column(
                children: <Widget>[
                  _typeContainer(
                      "생활환경", _getEnviorment(widget.petAddInfo['environment'])),
                  _typeContainer(
                      "운동강도", _getExercise(widget.petAddInfo['exercise'])),
                  _typeContainer(
                      "식사", _getFoodKind(widget.petAddInfo['foodKind'])),
                  _typeContainer(
                      "간식", widget.petAddInfo['snackCnt'].toString()),
                  _typeContainer("약", "O"),
                  SizedBox(
                    height: 10.h,
                  ),
                  if (widget.medicen.length > 0)
                    ...widget.medicen.map((medicine) {
                      return _mecicenContainer(
                          medicine['name'], medicine['cnt'].toString());
                    }).toList(),
                  // ToDo
                  // 약 동적생성해야 함
                ],

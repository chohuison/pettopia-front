...
class AI {
  ...
  String _getDefecation(String value){
    if(value =='NORMAL'){
      return 'NORMAL';
    }
    else{
  ...
  Future<Map<String, dynamic>> getPetDiseaseRecommendation(
      BuildContext context, int petPk) async {
    await _getServerUrl();

    Map<String, dynamic> petInfo = await _petServer.getPetInfo(context, petPk);
    Map<String, dynamic> petAddInfo =
        await _petServer.getAddPetInfo(context, petPk);
    ...
    String defecation =await  _diaryServer.getDiaryDefecation(context, petPk);
    ...
    final requestBody = {
      ...
      'environment':
          petOnlyAddInfo['environment'] == 0 ? "IN_DOOR" : "OUT_DOOR",
      'exercise': _getExersice(petOnlyAddInfo['exercise']),
      'defecation':_getDefecation(defecation),
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        print(
            "Failed to get pet disease recommendation. Status code: ${response.statusCode}");
        return {};
      }
    } catch (e) {
